...
const allTools = [
  ...
  { id: Tool.NewsDrafting, name: 'ئامادەكردنی هەواڵ', icon: <NewsDraftingIcon /> },
      ...
      <div className="w-full">
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
          {allTools.map((tool) => (
            ...
            >
              {tool.icon}
              <h2 className="text-xs font-semibold text-gray-200 h-10 flex items-center justify-center">{tool.name}</h2>
            </button>
          ))}
        </div>
      </div>
