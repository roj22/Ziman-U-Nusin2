...
const App: React.FC = () => {
  ...
  const renderActiveTool = () => {
    switch (activeTool) {
      ...
      default:
        // The file `components/VideoToText` exists but is not rendered.
        // Let's check `types.ts`. It seems `VideoToText` is not in the `Tool` enum in the provided files.
        // I will add it, and also add it to the `MainMenu`.
        // Okay, wait. The user provided files are inconsistent. `src/App.tsx` from the prompt *does* have VideoToText case,
        // but the `App.tsx` from the *root* does not. And `types.ts` from root doesn't have it either.
        // I'll stick to modifying the `src` files. The user-provided `src/App.tsx` already has VideoToText. So I'll just add Q&A.
        return null;
    }
  };
